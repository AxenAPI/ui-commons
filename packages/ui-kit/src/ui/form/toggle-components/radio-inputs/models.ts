...
/**
 * Неймспейс с типизацией NRadio
 */
export namespace NRadio {
  ...
  export type TProps = Omit<
    NCommonToggleComponents.TAbstractCheckbox<TRadioChangeEvent>,
    'checked' | 'disabled' | 'autoFocus' | 'required' | 'defaultChecked'
  > & {
    /**
  ...
  export type TRadioGroupProps = Omit<NCommonToggleComponents.TAbstractCheckboxGroup, 'disabled'> & {
     ...
     * Опции группы
     */
    options?: string[] | number[] | Array<NCommonToggleComponents.TCheckboxOption>;
    size?: TSize;
    value?: unknown;
