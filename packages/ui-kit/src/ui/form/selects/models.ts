...
import { NSegmented } from '@/ui/segmented/models';

/**
 * Неймспейс с типизацией Select
 */
export namespace NSelect {
  ...
  export type TOption = Omit<BaseOptionType, 'disabled'> & {
     ...
     */
    optionsSegmentedValue?: { [K in NSegmented.TItem['value']]: string };
  };
  ...
  export type TSelectOption = TOption | TOptionGroup;
  ...
  export type TCommonProps<TValue = any, TOptionProps extends BaseOptionType = DefaultOptionType> = {
    /**
     ...
     */
    filterOption?: boolean | ((inputValue: string, option?: TOptionProps) => boolean);
    /**
    ...
    /**
     * Массив объектов, каждый из которых соответствует опции и имеет структуру, описанную в `Option` или группе опций
     */
     ...
     * Обработчик добавления новго значения в селект
     */
    onChange?: (value: TValue, option?: TOptionProps | TOptionProps[]) => void;
    /**
    ...
    optionLabelProp?: string;
    /**
     * Параметр для режима чтения
     */
    isReadonly?: boolean;
  };

  export function isOptionGroup(option: TSelectOption): option is TOptionGroup {
    return (
      typeof option === 'object' &&
      option !== null &&
      'options' in option &&
      Array.isArray((option as TOptionGroup).options)
    );
  }
}
