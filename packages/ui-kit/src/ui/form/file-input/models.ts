...
import type { RcFile as OriRcFile, UploadRequestOption as RcCustomRequestOptions } from 'rc-upload/lib/interface';

/**
 * Неймспейс с типизацией FileInput
 */
export namespace NFileInput {
  ...
  type TUploadData<T> =
    ...
    | ((file: TUploadFile<T>) => Record<string, unknown> | Promise<Record<string, unknown>>);
  ...
  export type TDraggerProps = TProps & {
    /**
     * Иконка загрузки в Dragger
     */
    iconFile?: ReactNode;
    /**
     ...
     * Опсиание
     */
    descriptionFile?: ReactNode;
    /**
     * Стили для контейнера по умолчанию
     */
