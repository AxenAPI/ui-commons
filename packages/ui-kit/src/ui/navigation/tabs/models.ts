...
export namespace NTab {
  type TGetIndicatorSize = number | ((origin: number) => number);
  type TOnTabScroll = (info: { direction: 'left' | 'right' | 'top' | 'bottom' }) => void;
  ...
  type TTabBarExtraContent = React.ReactNode | TTabBarExtraMap;
  ...
  export type TTabsLabelWrapper = {
    ...
    extraNode?: { element: React.ReactNode; onHover?: boolean };
  };

  type TRenderTabBar = (
    props: TRenderTabBarProps,
    DefaultTabBar: React.ComponentType<TTabNavListProps>
  ) => React.ReactElement;
  ...
  export declare const TabNavList: React.ForwardRefExoticComponent<
    TTabNavListProps & React.RefAttributes<HTMLDivElement>
  >;
  ...
  export type TProps = {
     ...
     */
    tabBarExtraContent?: TTabBarExtraContent;
    /**
